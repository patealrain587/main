...
def XR(x, w):
    cols = [c for c in range(x, x + w) if c not in DEL]
    return X(cols[0]), len(cols)
def XP(pts):   # point list along a line: points on removed columns are dropped
    return [(X(px), py) for (px, py) in pts if px not in DEL]
def XS(xs_):  # iterable of x values: removed ones dropped
    return [X(v) for v in xs_ if v not in DEL]
...
def D(*pts):
    for p in pts: doors.append((X(p[0]), p[1]))
...
R("거주 통로", "", "corridor", 72, 10, 3, 36); D((73, 9))
...
DK("북서 격납 갑판", "북서 격납", -6, 1, 51, 21, "지붕 없음 · 전투기·센서 · 게임에서 서쪽 외곽까지 넓힘"); D((22, 22), (45, 10), (-3, 22))
# west outer strip built in game (x -6..-1, hull wall x -7): spare rooms chained off the NW hangar
R("서쪽 외곽 1", "외곽", "spare", -6, -9, 6, 9, "게임에서 지은 외곽 공실"); D((0, -2), (-3, -10))
R("서쪽 외곽 2", "외곽", "spare", -6, 23, 6, 22, "게임에서 지은 외곽 공실"); D((-3, 45))
R("서쪽 외곽 3", "외곽", "spare", -6, 46, 6, 9, "게임에서 지은 외곽 공실"); D((-3, 55))
R("서쪽 외곽 4", "외곽", "spare", -6, 56, 6, 26, "게임에서 지은 외곽 공실"); D((-3, 82))
R("서쪽 외곽 5", "외곽", "spare", -6, 83, 6, 9, "게임에서 지은 외곽 공실")
...
R("북측 에어록 1", "에어록", "airlock", 40, -10, 9, 6, "격벽 이중문"); D((44, -11))
# as built: corridor 1 continues past a door into a short hall, and a corridor under the NW battery joins it
R("북측 전실", "전실", "airlock", 46, -3, 3, 3, "세로 통로 1 끝 · 에어록 1 앞"); D((47, 0), (47, -4), (45, -2))
R("북서 연결 통로", "", "corridor", 1, -3, 44, 3)
R("북측 에어록 2", "에어록", "airlock", 99, -6, 9, 6, "격벽 이중문"); D((100, 0), (103, -7))
OUTER_DOORS = {(44, -11), (103, -7), (-3, -10)}   # unpowered doors: the defence grid may run under them
DK("북서 포대 갑판", "북서 포대", 1, -16, 38, 12, "지붕 없음 · 장거리 포탑 · 확장 예비지"); D((39, -8))
DK("북측 포대 갑판", "북측 포대", 50, -16, 48, 25, "지붕 없음 · 장거리 포탑 · 확장 예비지"); D((49, -8), (98, -3))
DK("북동 포대 갑판", "북동 포대", 109, -16, 49, 16, "지붕 없음 · 장거리 포탑 · 확장 예비지"); D((108, -3))
# defence power grid: north battery decks -> east hull wall -> entry deck, never touching the main grid
DEF_PORTALS = [(x, -11) for x in range(39, 50)] + [(x, -7) for x in range(98, 109)] + [(158, y) for y in range(-1, 110)]
...
for (px, py) in DEF_PORTALS + PORT_WALLS:
    for dy in (-1, 0, 1):
        for dx in (-1, 0, 1):
            q = (px + dx, py + dy)
            if K(*q) == INT or (K(*q) == DECK and (px, py) in DEF_PORTALS) or (K(*q) == DECK and (px, py) in PORT_WALLS):
                reserved.add(q)
...
for cx in range(57, 91, 3):
    if try_place(spec("AdvShip_GravThruster"), DO("추진기 갑판"), cx, 116, 0, check_reach=False): thr += 1
...
place_spread(DO("북서 격납 갑판"), interleave([["Milira_DragonFighter", "Milira_WyvernFighter", "Milira_DragonFighter", "Milira_WyvernFighter", "Milira_GriffinFighter", "Milira_GriffinFighter"],
                                            ["NCL_LaserDefenceTurret", "PLAMilira_Field_Tower_Player", "NCL_LaserDefenceTurret"], ["Milira_SunLightDefenceTowerII", "Milira_SunLightGatheringTower"]]),
             far_w=0, optional_keys=FIGHTERS)
# heliostats (open sky, link range 11.4) around the solar fuel gathering tower
for gt in [b for b in blds if b["sp"]["defName"] == "Milira_SunLightGatheringTower"]:
    gc = fp_centre(gt["cells"]); hn = 0
    for c in sorted(room_cells(gt["o"]), key=lambda c: abs(math.dist(c, gc) - 4)):
        if hn >= 4: break
        if math.dist(c, gc) <= 10 and try_place(spec("Milira_Heliostat"), gt["o"], c[0], c[1], 0): hn += 1
    if hn < 4: errors.append(f"only {hn} heliostats near the solar gathering tower")
...
for b in list(blds):
    ...
    for c in sorted(set(b["adj"]), key=lambda c: math.dist(c, cb)):
        ...
        if b["o"][0] == "d": errors.append(f"no small shield spot beside {b['sp']['name']} ({oname(b['o'])})")
...
for rn in LINK_ROOMS:
    ...
    for trial in range((LINK_TRIALS * 3 if sum(n for _, n in items) >= 4 else LINK_TRIALS) if items else 1):
